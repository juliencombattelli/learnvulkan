...
if(glslang IN_LIST Vulkan_FIND_COMPONENTS)
  find_library(Vulkan_glslang-default-resource-limits_LIBRARY
  ...
  find_library(Vulkan_glslang-default-resource-limits_DEBUG_LIBRARY
...
if(SPIRV-Tools-opt IN_LIST Vulkan_FIND_COMPONENTS)
  find_library(Vulkan_SPIRV-Tools-opt_LIBRARY
    NAMES SPIRV-Tools-opt
    HINTS
      ${_Vulkan_hint_library_search_paths})
  mark_as_advanced(Vulkan_SPIRV-Tools-opt_LIBRARY)

  find_library(Vulkan_SPIRV-Tools-opt_DEBUG_LIBRARY
...
# Remove MSVC ABI warning if variant missing because on Windows (and MinGW), dependencies impacted
# are built from source
_Vulkan_set_library_component_found(glslang-default-resource-limits NO_WARNING)
...
_Vulkan_set_library_component_found(glslang DEPENDENT_COMPONENTS glslang-spirv NO_WARNING)
_Vulkan_set_library_component_found(shaderc_combined NO_WARNING)
_Vulkan_set_library_component_found(SPIRV-Tools NO_WARNING)
_Vulkan_set_library_component_found(SPIRV-Tools-opt NO_WARNING)
_Vulkan_set_library_component_found(volk NO_WARNING)
_Vulkan_set_library_component_found(dxc NO_WARNING)
...
if(Vulkan_FOUND)
  if((Vulkan_glslang-default-resource-limits_LIBRARY OR Vulkan_glslang-default-resource-limits_DEBUG_LIBRARY) AND NOT TARGET Vulkan::glslang-default-resource-limits)
    add_library(Vulkan::glslang-default-resource-limits STATIC IMPORTED)
    set_property(TARGET Vulkan::glslang-default-resource-limits
      PROPERTY
        INTERFACE_INCLUDE_DIRECTORIES "${Vulkan_INCLUDE_DIRS}")
    if(Vulkan_glslang-default-resource-limits_LIBRARY)
      set_property(TARGET Vulkan::glslang-default-resource-limits APPEND
        PROPERTY
          IMPORTED_CONFIGURATIONS Release)
      set_property(TARGET Vulkan::glslang-default-resource-limits
        PROPERTY
          IMPORTED_LOCATION_RELEASE "${Vulkan_glslang-default-resource-limits_LIBRARY}")
    endif()
    if(Vulkan_glslang-default-resource-limits_DEBUG_LIBRARY)
      set_property(TARGET Vulkan::glslang-default-resource-limits APPEND
        PROPERTY
          IMPORTED_CONFIGURATIONS Debug)
      set_property(TARGET Vulkan::glslang-default-resource-limits
        PROPERTY
          IMPORTED_LOCATION_DEBUG "${Vulkan_glslang-default-resource-limits_DEBUG_LIBRARY}")
    endif()
  endif()
  ...
  if((Vulkan_SPIRV-Tools-opt_LIBRARY OR Vulkan_SPIRV-Tools-opt_DEBUG_LIBRARY) AND NOT TARGET Vulkan::SPIRV-Tools-opt)
    add_library(Vulkan::SPIRV-Tools-opt STATIC IMPORTED)
    set_property(TARGET Vulkan::SPIRV-Tools-opt
      PROPERTY
        INTERFACE_INCLUDE_DIRECTORIES "${Vulkan_INCLUDE_DIRS}")
    if(Vulkan_SPIRV-Tools-opt_LIBRARY)
      set_property(TARGET Vulkan::SPIRV-Tools-opt APPEND
        PROPERTY
          IMPORTED_CONFIGURATIONS Release)
      set_property(TARGET Vulkan::SPIRV-Tools-opt
        PROPERTY
          IMPORTED_LOCATION_RELEASE "${Vulkan_SPIRV-Tools-opt_LIBRARY}")
    endif()
    if(Vulkan_SPIRV-Tools-opt_DEBUG_LIBRARY)
      set_property(TARGET Vulkan::SPIRV-Tools-opt APPEND
        PROPERTY
          IMPORTED_CONFIGURATIONS Debug)
      set_property(TARGET Vulkan::SPIRV-Tools-opt
        PROPERTY
          IMPORTED_LOCATION_DEBUG "${Vulkan_SPIRV-Tools-opt_DEBUG_LIBRARY}")
    endif()
  endif()

  if((Vulkan_glslang_LIBRARY OR Vulkan_glslang_DEBUG_LIBRARY)
      AND TARGET Vulkan::glslang-spirv
      AND TARGET Vulkan::SPIRV-Tools-opt
      AND TARGET Vulkan::SPIRV-Tools
